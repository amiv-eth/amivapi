...
class TestServer:
    ...
    def __enter__(self):
        ...
        try:
            requests.get(apiurl + '/')
            raise ServerError('A server is already running. Are you debugging?')
        ...
        # prexec_fn=setsid makes the process and all its children part of a group
        # so we can kill them all as one later
        ...
        while(self.serverProcess.poll() == None): 
            try:
        ...
        raise ServerError('Could not start development server! Exit code: ' +
