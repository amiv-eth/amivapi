...
class EventiCalendarTest(WebTestNoAuth):
    def test_events_json_ical_3(self):
        ev0 = {
            'title_en': 'party',
            'description_en': 'fun',
        ...
        r = jloads(self.api.get("/events", status_code=200).get_data(as_text=True))
        d = r['_items']
        ...
        r = self.api.get("/events", headers={'Accept': 'text/calendar'}, status_code=200).get_data(as_text=True)
    ...
    def test_events_json_ical_1_noendtime(self):
        ...
        for c in g.walk():
            if c.name == "VEVENT":
                ...
                self.assertEqual(c.get('dtend').dt, testtime+timedelta(minutes=60))
                count += 1
        self.assertEqual(count, 1)
    ...
    def test_events_json_ical_1_idendpoint(self):
        ...
        # get iCalendar data and check if etag matches uid
        r = self.api.get('/events/'+str(ev_id), headers={'Accept': 'text/calendar'}, status_code=200).get_data(as_text=True)
        g = Calendar.from_ical(r)
        self.assertEqual(g['version'], '2.0')
        count = 0
        for c in g.walk():
