...
Code:
    ...
    response = requests.post("https://api.amiv.ethz.ch/sessions", data={"user": "mynethzormail", "password": "mypassword"})
...
Request:
    ...
    POST /sessions "nethz=mynethzormail&password=mypassword"
    GET /users/mynethz (+Authorization header)

Code with good REST library:

    login = requests.post("http://api.amiv.ethz.ch/sessions", data={"user": "myuser", "password": "mypassword"})
...
Code with bad REST library:
    ...
    login = requests.post("/sessions", data={"user": "myuser", "password": "mypassword"})
    token = login.json()['token']
