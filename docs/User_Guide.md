...
Request:
    ...
    POST /sessions?nethz=myuser&password=mypassword

Code:

    response = requests.post("https://api.amiv.ethz.ch/sessions", data={"nethz": "myuser", "password": "mypassword"})
...
Instead of nethz it is also possible to login with your email.

Code:

    response = requests.post("https://api.amiv.ethz.ch/sessions", data={"email": "myemail@amiv.ethz.ch", "password": "mypassword"})
...
Code with good REST library:
    ...
    login = requests.post("http://api.amiv.ethz.ch/sessions", data={"nethz": "myuser", "password": "mypassword"})
...
Code with bad REST library:
    ...
    login = requests.post("/sessions", data={"nethz": "myuser", "password": "mypassword"})
    token = login.json()['token']
