...
config_path = join(settings.ROOT_DIR, "mongo_config.cfg")
...
def create_config(force=False,
    ...
    if not db_type:
        db_type = prompt_choices("Choose the type of database",
                                 ["mongo"],
                                 default="mongo")
    elif db_type not in ['mongo', 'mongo']:
        print("Only mongo supported as db type!")
        exit(0)
    ...
    if db_type == "sqlite":
        ...
        db_user = prompt("MongoDB username")
        db_pass = prompt_pass("MongoDB password", default="")
        db_host = prompt("MongoDB host", default="localhost")
        db_port = prompt("MongoDB port", default=27017)
        db_name = prompt("MongoDB database")

        config['MONGO_HOST'] = db_host
        config['MONGO_PORT'] = db_port
        config['MONGO_USERNAME'] = db_user
        config['MONGO_PASSWORD'] = db_pass
        config['MONGO_DBNAME'] = db_name
